...
export async function generateStaticParams() {
  const res = await fetch('https://api.github.com/repos/kmustafa0/blog-posts/contents/posts');
...
export async function generateMetadata({
  ...
  const siteUrl = process.env.NEXT_PUBLIC_SITE_URL || 'https://superfast.vercel.app';
  ...
  return {
    ...
    openGraph: {
      ...
      images: [
        {
          url: `${siteUrl}/api/og?slug=${encodeURIComponent(post.metadata.title)}`,
          width: 1200,
          height: 630,
    ...
    twitter: {
      ...
      title: post.metadata.title,
      description: post.metadata.excerpt,
      images: [`${siteUrl}/api/og?slug=${encodeURIComponent(post.metadata.title)}`],
    },
  };
}
...
export default async function PostPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <article className='prose prose-invert max-w-none'>
      ...
      <time className='text-sm text-gray-500'>{formatDateEU(post.metadata.date)}</time>
